...
Start the observability dashboard server with Cursor platform support.
...
**Step 5 — Set platform environment and start the server**

Set the platform environment variable for Cursor and start the server:

```bash
PLATFORM=cursor node .agent/dashboard/server.js &
```

Then print:

```
Dashboard started with Cursor platform support. Open http://localhost:3000 in your browser. (actual port may vary if 3000 is in use — check the output above)

The dashboard will show:
- Current platform: Cursor
- Manual subagent invocation workflow
- Shared artifacts and logging from both platforms
- Step-by-step guidance for manual orchestration
```
...
- Sets PLATFORM=cursor environment variable for platform-aware dashboard
- Enhanced messaging about Cursor-specific features
- Same shared dashboard server with platform detection